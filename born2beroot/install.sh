...
_provisioning() {
    # fix permissions /tmp
    chmod 1777 ${_MOUNT_PATH}/rootfs/tmp
    cp ${_ROOT_DIRECTORY}/auto-install.sh ${_MOUNT_PATH}/rootfs/tmp/auto-install.sh
    chmod +x ${_MOUNT_PATH}/rootfs/tmp/auto-install.sh

    _ADDUSER=${_USERNAME} _DISKMAP=${_LOOP} _CRYPTMAP=${_CRYPT_NAME} chroot ${_MOUNT_PATH}/rootfs /tmp/auto-install.sh

    cp -R ${_ROOT_DIRECTORY}/rootfs/* ${_MOUNT_PATH}/rootfs/.

    echo "{_USERNAME}   ALL=(ALL) NOPASSWD: /usr/local/bin/monitoring.sh" >> ${_MOUNT_PATH}/rootfs/etc/sudoers.d/00-allow-command
    _set_hostname
    _set_hosts
    ...
    ${_INITRAMFS} && {
        mkdir -p ${_MOUNT_PATH}/rootfs/usr/share/initramfs-tools
        cp -r $(pwd)/initramfs/* ${_MOUNT_PATH}/rootfs/usr/share/initramfs-tools/.
    }
}
...
_set_apt_configuration() {
    mkdir -vp ${_MOUNT_PATH}/rootfs/etc/apt/trusted.gpg.d
    cp -v /etc/apt/trusted.gpg.d/debian-archive*.gpg ${_MOUNT_PATH}/rootfs/etc/apt/trusted.gpg.d/.
...
_set_fstab() {
    ...
    for n in $(seq 0  $((${#_LVM_LOGICAL_VOLUME[@]} - 1))); do 
        ...
        local _tag=${lvm_volume##*--}
        local _device="/dev/${_LVM_GROUP}/${lvm_volume}"
        local _uuid=$(blkid ${_device} | grep -Eo  'UUID=(\"[0-9a-fA-F-]+\") ')

        [ "${_tag}" == "swap" ] && {
            local _fstab="${_fstab}${_uuid}\t\t\tnone\t\t\tswap\t\t\tsw\t\t\t0\t\t\t0\n"
            continue 
        }
        
        [ "${_tag}" == "root" ] && {
            local _fstab="${_fstab}${_uuid}\t\t\t/\t\t\text4\t\t\terrors=remount-ro\t\t\t0\t\t\t1\n"
            continue
        }

        local _fstab="${_fstab}${_uuid}\t\t\t/${_tag/-/\/}\t\t\text4\t\t\tdefaults\t\t\t0\t\t\t2\n"
    done 

    local _uuid="$(blkid /dev/mapper/${_LOOP##*/}p1 | grep -Eo  'UUID=(\"[0-9a-fA-F-]+\") ')"
    local _fstab="${_fstab}${_uuid}\t\t\t/boot\t\t\tvfat\t\t\tumask=0077\t\t\t0\t\t\t1\n"
